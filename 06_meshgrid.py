        ...
        for num_ones_removed in range(np.min([distance, ones_count]) + 1):
            for clear_rows in combinations(one_pos[0], num_ones_removed):
                ...
                for rows in combinations(remained_rows, distance - num_ones_removed):
                    ...
                    col_valid_indices = []
                    for row in rows:
                        col_valid_indices.append(list(range(where_one[row])) +
                                                 list(range(where_one[row] + 1, k)))
                    row_offsets = np.array(rows, dtype=np.int32)*k

                    r = np.array(np.meshgrid(*col_valid_indices)).T.reshape(-1, len(col_valid_indices))
                    r += row_offsets
